...
## 레이아웃  
> 가이드 제작시 레이아웃의 가변영역에 대한 내용 PSD에 표시

### 레이아웃의 흐름에 맞게 콘텐츠를 배치하자.
> 웹사이트에서 레이아웃이 구성되는 방식은 왼쪽에서 오른쪽으로 상단에서 하단으로 쌓이게 됩니다. [예제](https://codepen.io/blythe4/pen/ZPZQLd)   
> 웹부터 디자인을 시작 한다면 모바일을 염려해두고 레이아웃의 흐름을 생각하면서(왼쪽에서 오른쪽으로, 상단에서 하단으로) 진행 하는게 좋을거 같습니다.
...
* 해상도는 사용자마다 브라우저의 설정이 다르기 때문에 화면에 넘치거나, 여백이 많이 생길 수 도 있습니다.
* Web의 최소 높이 사이즈를 HD 해상도로 생각하고 사용자의 브라우저 설정에 따라서 100px ~ 200px정도 차이가 있어 컨텐츠의 높이는 최대 550px이 넘지 않도록 합니다.
* Web에서 Fullpage로 하고 다른 디바이스에서 일반 스크롤이 되도록 하고, 컨텐츠는 항상 화면의 수직 가운데 정렬이 되도록 합니다.
...
> * 예를 들어 33px * 33px의 아이콘을 퍼블리싱 할때 16px * 16px 이나 17px * 17px 둘 중 한가지를 선택하여 코딩하게 됩니다.
...
1. 화면에 꽉차게 표시  
    ...
    : 디바이스 화면의 비율에 따라 가로보다 세로의 너비가 클때(320x568) 또는 세로보다 가로의 너비가 클때(568x320) 표시되어야 하는 동영상이 각각 필요합니다.
